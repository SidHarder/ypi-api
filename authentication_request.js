...
const payload = {
    "sub": "YPI Connect Authentication",
    "name": "John Doe",
    "reportBrowser": true
  }
...
function processAuthenticationRequest (args, cb) {
...
function getToken (args, cb) {    
    ...
    IsUsernamePasswordValid(userName, password, function (error, isValid, webServiceAccount) {
        ...
        if (isValid == true) {
            ...
            if (verified) {
                var token = jwt.sign(webServiceAccount, webServiceAccount.TwoFactorSecret)
...
function updateSecret (secret, webServiceAccount, cb) {    
    let commandText = `Update tblWebServiceAccount set TwoFactorSecret = '${secret.ascii}' where WebServiceAccountId = '${webServiceAccount.WebServiceAccountId}'`
...
authenticationRequest.processAuthenticationRequest = processAuthenticationRequest
