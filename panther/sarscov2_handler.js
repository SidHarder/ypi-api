  ...
  db.executeSqlCommand(sql, function (error, result) {
    ...
    if (mappedResult.code == 'SARSCOV2PSTV') {
      ...
      if (clientId == 1805) {
        ...
        textMessage.sendMultiple([{ phoneNumbers: ['4065462446', '4063965675'], message: 'YPI ALERT: A Positve COVID result for your organization has been detected.' }], function (error, result) {
          if (error) {
            console.error(error);
            return cb(null, error);
          }
